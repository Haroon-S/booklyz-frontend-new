  ...
  return (
    <Grid container>
      ...
      <Grid item xl={6} lg={6} md={6} sm={12} xs={12} borderLeft={1} p={4}>
        <Typography variant="h4">Journals entries</Typography>

        <Box display="flex" gap={2} mt={2}>
          {!loading && journalData?.results?.length > 0 && (
            <Link
              href={`/portal/owner/journals/patients/${id}/journal?tab=tab1&journal=${journalData?.results[0]?.id}`}
            >
